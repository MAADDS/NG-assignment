...
void main()
{
	...
	if( socketListen == INVALID_SOCKET ) // 에러가 아니면 -1
	{
	...
	serverAddr.sin_addr.s_addr	= ::htonl( INADDR_ANY ); // ::inet_addr( "165.194.115.25" ); 
							//::htonl( INADDR_LOOPBACK  ); //::inet_addr( "127.0.0.1" ); 클라이언트에선 이걸 사용
	...
	while( 1 )
	{
		...
		if( socketClient == INVALID_SOCKET )
		{
			...
			if( FD_ISSET( socketListen, &fds ) )
			{
				...
				socketClient = ::accept( socketListen, ( struct sockaddr* )&fromAddr, &size ); //from add라는 주소, accept 해서 새로운 소켓을 만듬
