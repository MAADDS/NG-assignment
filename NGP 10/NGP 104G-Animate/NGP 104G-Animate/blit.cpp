...
void DoBlitExample (void)
{
	...
	if (dwBlitExamples == SHRINK_N_GROW)
	{
		...
		static int ShrinkOrGrowNum = 1;
		static int SGrange = 80;

		rcDest.left = (DWORD)(640 / 2 - 4 * ShrinkOrGrowNum);
		rcDest.top = (DWORD)(480 / 2 - 3 * ShrinkOrGrowNum);
		rcDest.right = (DWORD)(640 / 2 + 4 * ShrinkOrGrowNum);
		rcDest.bottom = (DWORD)(480 / 2 + 3 * ShrinkOrGrowNum);

		ddrval = lpDDSBack->Blt(&rcDest, lpDDSFace, NULL, NULL, NULL);
		
		if (ShrinkOrGrowNum == SGrange || ShrinkOrGrowNum == 0)
